const SenderKeyMessage = require('../protocol/SenderKeyMessage');
const aesgcm = require('../../common/aesgcm');
...
class GroupCipher {
  ...
  getPlainText(iv, key, ciphertext) {
    try {
      const plaintext = aesgcm.decryptCBC(key, iv, ciphertext);
      return plaintext;
    } catch (e) {
      console.log(e.stack);
      throw new Error('InvalidMessageException');
  ...
  getCipherText(iv, key, plaintext) {
    try {
      ...
      const crypted = aesgcm.encryptCBC(key, iv, Buffer.from(plaintext));
      return crypted;
    } catch (e) {
      console.log(e.stack);
      throw new Error('InvalidMessageException');
